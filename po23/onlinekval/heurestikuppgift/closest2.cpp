...
int main()
{
    ...
    for (int try_num = 0, stop1 = min(num_tries[T], N); try_num < stop1; try_num++) {
        ...
        //try_swap2(path, N/2, x, y, 1, 3);
        p_len = 0;
        rep(i, 0, N - 1) {
            p_len += abs(x[i] - x[i + 1]) + abs(y[i] - y[i + 1]);
        }
...
void t1(int T, int N) {
    ...
    int min_path_len = INF;
    vi best_path;
    rep(start, 0, min(100, N/2)) {
        int p_len = 0;
        vi path = {points[start][2]};
        int cur = start;
        // add points
        for(int to = cur + 1, stop = (cur + N/2); to < stop; to++) {
            p_len += abs(points[cur][0] - points[to][0]) + abs(points[cur][1] - points[to][1]);
            path.pb(points[to][2]);
            cur = to;
        }
        try_swap(path, x, y, 5, 20);
...
void t2(int T, int N)
{
    ...
    int num_tries[11] = {10, 1, 1, 10, 40, 200, 1000, 60, 6, 1, 1};
    int seeds[11] = {0, 20000, 80000, 100, 100, 2000, 80000, 80000, 80000, 80000, 80000};
